...
import com.w83ll43.musicplayer.databinding.ActivityPlayerBinding
import com.w83ll43.musicplayer.extension.goneAlphaAnimation
import com.w83ll43.musicplayer.extension.toMinAndSeconds
import com.w83ll43.musicplayer.extension.visibleAlphaAnimation
import com.w83ll43.musicplayer.logic.dao.SharedViewModel
import com.w83ll43.musicplayer.service.MusicService
import com.w83ll43.musicplayer.ui.MainActivity
import com.w83ll43.musicplayer.ui.comment.CommentActivity
...
class PlayerActivity : BaseActivity() {
    ...
    override fun initObserver() {
        ...
        // 歌词发生变化
        viewModel.lyricLiveData.observe(this, Observer {
        ...
        // 监听音乐歌曲发生变化
        shareViewModel.nowPlayerSongLiveData.observe(this, Observer {
            val item = it.value
            println("nowPlayerSongLiveData $item")
            binding.apply {
            ...
            Glide.with(this).apply {
                load(item?.al?.picUrl).apply(RequestOptions.bitmapTransform(BlurTransformation(60, 60))).into(bgView)
                load(R.drawable.play_disc).apply(RequestOptions.bitmapTransform(CircleCrop())).into(playDisc)
                load(item?.al?.picUrl).apply(RequestOptions.bitmapTransform(CircleCrop())).into(writeImg)
            }
            item?.id?.let { songId: Int -> viewModel.getLyric(songId) }
        })
        //监听播放状态
        shareViewModel.playStatusLiveData.observe(this, Observer {
            when (it) {
                12 -> {
    ...
    override fun initListener() {
        ...
        binding.apply {
            ...
            //根据传递的参数判断是从 music_tab 进来的还是，从播放歌单进来的
            if(params !="yes"){
                //进入立即播放
                Timer().schedule(200) {
                    createPlay(intent)
    ...
    companion object {
        const val CTL_ACTION = "com.w83ll43.musicplayer.MUSIC_BROADCAST"
        const val UPDATE_ACTION = "com.w83ll43.musicplayer.UPDATE_ACTION"
