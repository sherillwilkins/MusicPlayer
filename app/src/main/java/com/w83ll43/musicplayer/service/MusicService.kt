package com.w83ll43.musicplayer.service
...
import com.w83ll43.musicplayer.NotificationReceiver
...
class MusicService : Service() {
    ...
    private lateinit var serviceReceiver: MusicServiceReceiver
    ...
    override fun onCreate() {
        ...
        serviceReceiver = MusicServiceReceiver()
        // 创建 IntentFilter
    ...
    private fun prepareAndPlay() {
        ...
        if (musicsUrl?.url != null) {
            ...
            info.apply {
                ...
                if (musicInfo.ar != null) {
                    artist = "${musicInfo.ar[0].name} - ${musicInfo.al.name}"   //作者
                } else {
                    artist = musicInfo.album.artist.name   // 作者
                }
                if (musicInfo.al != null) {
                    songCover = musicInfo.al.picUrl  // 音乐封面
                } else {
                    songCover = musicInfo.album.artist.img1v1Url  // 音乐封面
                }

                songId = musicsUrl.id.toString()
                songUrl = musicsUrl.url
            }
            ...
            XToastUtils.error("暂无播放源，请切换其他歌曲");
        }
    }

    inner class MusicServiceReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent) {
            ...
            // 初始化通知栏
            initNotificationBar()
            // 1.上下文 2.图标控件的ResID 3.RemoteView 4 Notification 5 Notification_ID
    ...
    * */
    ...
    private fun initNotificationBar() {
        ...
        val builder: Notification.Builder? // 建立一个通知消息的构造器
